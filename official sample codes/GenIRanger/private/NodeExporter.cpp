...
void NodeExporter::enterCategory(const CCategoryPtr& node)
{
  mFormat.outputCategory(std::string(node->GetNode()->GetName()));
  mFormat.pushCategory();
}

void NodeExporter::leaveCategory(const CCategoryPtr& category)
{
  mFormat.popCategory();
}

void NodeExporter::enterSelector(const CNodePtr& selector)
{
  mFormat.pushSelector(selector);
}

void NodeExporter::leaveSelector(const CNodePtr& node)
{
  mFormat.popSelector();
}

void NodeExporter::onLeaf(const CNodePtr& node)
{
  ...
  {
    std::stringstream ss;
    ss << "Cannot read " << name << ". Library exception: "
       << e.GetDescription();
