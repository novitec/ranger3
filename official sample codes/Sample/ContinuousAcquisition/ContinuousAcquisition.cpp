...
void usage(int, char* argv[])
{
  ...
  std::cout << std::endl;
  std::cout << "Path to the directory where buffers should be saved" << std::endl
...
void BufferPartInfo::log(std::ostream& logFile) const
{
  ...
  case GenTL::PART_DATATYPE_3D_IMAGE:
    ...
    break;
  default:
    logFile << " Unknown Part " << mPartDataType;
    break;
  }
  ...
  case PFNC_Coord3D_C16:
    ...
    break;
  case PFNC_Mono16:
    logFile << " Format: DATA_FORMAT_MONO_16 ";
    break;
  case PFNC_Coord3D_C12p:
    ...
    break;
  case PFNC_Mono12p:
    logFile << " Format: DATA_FORMAT_MONO_12p ";
    break;
  default:
    logFile << " Unknown Format  " << mPartDataFormat;
    break;
  }
  logFile << std::endl;
}
...
int main(int argc, char* argv[])
{
  ...
  if (!Sample::parseArgumentHelper(argc, argv, "directory to save buffers in",
                                   savePath))
  {
    usage(argc, argv);
    return 1;
  ...
  *regionSelector = "Region1";
  ...
  std::string filename
    = filePath + "\\log-" + Sample::currentDateTime() + ".txt";
  ...
  // Start acquisition
  CC(tl, tl->DSStartAcquisition(dataStreamHandle, GenTL::ACQ_START_FLAGS_DEFAULT,
                                GENTL_INFINITE));
  ...
  try
  {
    ...
    for (int64_t i = 1; i < numToSave + 1; i++)
    {
      ...
      try
      {
        ...
        BufferInfo bufferInfo(tl, dataStreamHandle, bufferHandles[bufferId]);
        assert(bufferInfo.mBufferPayloadType == GenTL::PAYLOAD_TYPE_MULTI_PART);
        assert(bufferInfo.mPartCount == 2);

        BufferPartInfo part0Info(tl, dataStreamHandle,
                                 bufferHandles[bufferId], 0);
        ...
        BufferPartInfo part1Info(tl, dataStreamHandle,
                                 bufferHandles[bufferId], 1);
        ...
        if (saveToDisk)
        {
          ...
          bufferPath << savePath << "\\" << bufferName << "-"
            << (bufferRotationNum == 0 ? i :
                (i % bufferRotationNum + 1 == 0
                 ? 1 : i % bufferRotationNum + 1));
    ...
    logFile << std::endl
            ...
            << discardedBlockCount->GetValue() << std::endl
    ...
    for (size_t i = 0; i < buffersCount; i++)
    {
      ...
      CC(tl, tl->DSRevokeBuffer(dataStreamHandle, bufferHandles[i],
