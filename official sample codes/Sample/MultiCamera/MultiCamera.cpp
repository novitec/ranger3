...
void usage(int, char* argv[])
{
  std::cout << "Usage:" << std::endl
    ...
    << "Path to the directory where buffers should be saved" << std::endl
    << "E.g. C:\\MyBuffers\\buffer" << std::endl << std::endl;
}
...
public:
  ...
  void initializeBuffers(size_t buffersCount,
                         size_t payloadSize);
...
void DeviceConnection::createDeviceNodeMap(Sample::Consumer& consumer)
{
  GenTL::PORT_HANDLE devicePort;
  CC(mTl, mTl->DevGetPort(mDeviceHandle, &devicePort));
  mDevicePort.reset(new Sample::GenTLPort(devicePort, mTl));
  mDeviceNodeMap = consumer.getNodeMap(mDevicePort.get(), "Device");
}

void DeviceConnection::createDataStreamNodeMap(Sample::Consumer& consumer)
{
  mDataStreamPort.reset(new Sample::GenTLPort(mDataStreamHandle, mTl));
...
{
  CC(mTl, mTl->GCRegisterEvent(mDataStreamHandle,
                               GenTL::EVENT_NEW_BUFFER,
                               &mNewBufferEventHandle));
                          ...
                          GenTL::BUFFER_HANDLE bufferHandle)
{
  ...
  CC(tl, tl->DSGetBufferInfo(dataStreamHandle,
                             bufferHandle,
...
void logBufferInformation(GenTLApi* tl,
                          ...
                          GenTL::BUFFER_HANDLE bufferHandle)
{
  logBufferInformation(tl,
                       device.mLog,
                       device.mDataStreamHandle,
                       bufferNum,
                       bufferHandle);
}
...
void setThreadPriority(GenApi::CNodeMapRef &dataStreamNodeMap)
{
  ...
  {
      std::cout << "WARNING: Could not set thread priority!" << std::endl;
  }
...
void printStatistics(GenApi::CNodeMapRef& dataStreamNodeMap, std::ostream& out)
{
  ...
  out << "Buffer acquisition statistics End" << std::endl
      ...
      << "Seen Packet Count: "
      << seenPacketCount->GetValue() << std::endl
      << "Lost Packet Count: "
      << lostPacketCount->GetValue() << std::endl
      << "Delivered Packet Count: "
      << deliveredPacketCount->GetValue() << std::endl
      << "Unavailable Packet Count: "
      << unavailablePacketCount->GetValue() << std::endl
      << "Duplicate Packet Count: "
      << duplicatePacketCount->GetValue() << std::endl
      << "Resend Packet Count: "
      << resendPacketCount->GetValue() << std::endl
      << "Skipped Block Count: "
      << skippedBlockCount->GetValue() << std::endl
      << "Engine Underrun Count: "
      << engineUnderrunCount->GetValue() << std::endl
      << "Discarded Block Count: "
      << discaardeBlockCount->GetValue() << std::endl
      << "Incomplete Block Count: "
      << incompleteBlockCount->GetValue() << std::endl
      << "Oversized Block Count: "
      << oversizedBlockCount->GetValue() << std::endl
      //<< "Stream Thread Prio: "
                                  ...
                                  const std::string& path)
{
  ...
  GenIRanger::convert12pTo16(buffer12Data,
                             buffer12Size,
                             buffer16Data,
                             &buffer16Size);
                         ...
                         std::shared_ptr<DeviceConnection> deviceConnection)
{
  ...
  for (;;)
  {
    ...
    if (result == GenTL::GC_ERR_SUCCESS)
    {
      ...
      // Re-queue buffer
      CC(tl, tl->DSQueueBuffer(deviceConnection->mDataStreamHandle,
                               bufferHandle));
    }
...
int main(int argc, char* argv[])
{
  ...
  do {
    ...
    Sample::DeviceList devices = consumer.getDevices(interfaceHandle);
    i=0;
    ...
    CC(tl, tl->IFGetDeviceInfo(interfaceHandle,
                               deviceId.c_str(),
    ...
    connectedDevices.push_back(
      std::make_shared<DeviceConnection>(tl,
                                         deviceHandle,
                                         dataStreamHandle,
                                         deviceName.str()));
       ...
       ++it)
  {
    ...
    setThreadPriority(deviceConnection->mDataStreamNodeMap);
    ...
    // A temporary buffer used for 12-to-16-bit conversion before saving to disk.
  ...
  std::cout << "Total memory used for buffers: "
            << (totalAllocatedMemory / 1024 / 1024) << " MB"
            << std::endl;
  ...
  for (int j = 1; j <= startStopIterationsCount; ++j)
  {
         ...
         ++it)
    {
      ...
      std::cout << "Starting acquisition of "
                << (*it)->mDeviceName
                << std::endl;
    ...
    for (size_t i = 1; i < numBuffersToAcquire + 1; i++)
    {
           ...
           ++it)
      {
        ...
        try
        {
          ...
          GenTL::EVENT_NEW_BUFFER_DATA event;
          size_t eventSize = sizeof(event);
          CC(tl, tl->EventGetData(deviceConnection->mNewBufferEventHandle,
                                  &event,
                                  &eventSize,
                                  timeout));
          ...
          GenTL::INFO_DATATYPE dataType;
          CC(tl, tl->DSGetInfo(deviceConnection->mDataStreamHandle,
          ...
          GenApi::CIntegerPtr engineUnderrunCount = deviceConnection
            ->mDataStreamNodeMap._GetNode("GevStreamEngineUnderrunCount");
          if (engineUnderrunCount.IsValid())
          {
            ...
            {
              std::cout << "U";
              deviceConnection->mLog << "StreamEngineUnderrunCount: "
                                     << currentUnderrunCount << std::endl;
            }
         ...
         ++it)
    {
      ...
      CC(tl, tl->DSStopAcquisition(dataStreamHandle,
                                   GenTL::ACQ_STOP_FLAGS_KILL));
      GenApi::CIntegerPtr paramsLocked = device._GetNode("TLParamsLocked");
