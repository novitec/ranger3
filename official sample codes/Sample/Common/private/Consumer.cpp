...
LocalUrl parseLocalUrl(const std::string &url)
{
  ...
  localUrl.length = static_cast<size_t>(_strtoui64(tokens[2].c_str(),
                                                   &endptr_len, 16));
  return localUrl;
}
...
GenTL::IF_HANDLE Consumer::openInterfaceByName(std::string& interfaceName)
{
  ...
  {
    std::cout << "Cannot find interface with name "
                                             ...
                                             std::string& deviceName)
{
  ...
  CR(mTl->IFOpenDevice(interfaceHandle, deviceId.c_str(),
                       deviceAccess, &deviceHandle));
                                           ...
                                           DeviceId& deviceId)
{
  ...
  GenTL::DEVICE_ACCESS_FLAGS deviceAccess = GenTL::DEVICE_ACCESS_CONTROL;
  GenTL::DEV_HANDLE deviceHandle;
  GenTL::GC_ERROR status = mTl->IFOpenDevice(interfaceHandle, deviceId.c_str(),
                                             deviceAccess, &deviceHandle);
  ...
  {
    std::cout << "Connecting to device " << deviceId << std::endl
              << std::endl;
  }
...
std::string Consumer::getInterfaceName(InterfaceList interfaces, InterfaceId& id)
{
  for (auto it = interfaces.begin(); it != interfaces.end(); ++it)
  {
...
DeviceList Consumer::getDevices(GenTL::IF_HANDLE ifHandle)
{
  DeviceList devices;
  bool8_t changed = false;
  CR(mTl->IFUpdateDeviceList(ifHandle, &changed, 100));
  uint32_t numDevices;
  ...
  for (size_t i = 0; i < numDevices; ++i)
  {
    ...
    CR(mTl->IFGetDeviceID(ifHandle, static_cast<uint32_t>(i),
...
LocalUrl Consumer::getNodeMapUrl(GenTL::PORT_HANDLE hRemote)
{
  ...
  CR(mTl->GCGetPortURLInfo(hRemote, 0, GenTL::URL_INFO_URL,
                           &dataType, &url, &urlSize));
...
std::string Consumer::getNodeMapXML(GenTL::PORT_HANDLE hRemote)
{
  ...
  CR(mTl->GCReadPort(hRemote, localUrl.address,
                     &xmlContents.front(), &xmlContentsSize));
