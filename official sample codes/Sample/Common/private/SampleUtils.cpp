...
std::string getPathToProducer()
{
  ...
  {
    std::cout << "Could not locate SICKGigEVisionTL.cti." << std::endl
...
static bool handleArgument(int& argc, char *argv[], int idx,
                           const std::string& argName,
                           std::string &result, bool& failure)
{
                              ...
                              DeviceSelector& selector)
{
  ...
  while (idx < argc)
  {
    ...
    if (handleArgument(argc, argv, idx, "--mac", macAddr, failure))
    {
      ...
      int a, b, c, d, e, f;
      char dummy; // To make sure there is nothing after the last part of the MAC
      if (sscanf(macAddr.c_str(), "%x:%x:%x:%x:%x:%x%c",
                 &a, &b, &c, &d, &e, &f, &dummy) == 6)
      {
        int sanityCheck = (a | b | c | d | e | f);
        if ( sanityCheck != (sanityCheck & 0xff) )
        {
          std::cerr << "ERROR: " << macAddr
            << " is not a legal MAC address" << std::endl;
          failure = true;
        }
      ...
      {
        std::cerr << "ERROR: \"" << macAddr
          << "\" is not a valid MAC address" << std::endl;
        failure = true;
      }
    }
...
void printDeviceOptionsHelp()
{
  ...
  std::cout << "   --mac xx:xx:xx:xx:xx:xx   Connect to the device "
    "with the specified mac" << std::endl;
  std::cout << "   --ip x.x.x.x              Connect to the device "
    "with the specified ip" << std::endl;
}
                         ...
                         std::string& argumentValue)
{
  ...
  std::cout << "No argument given for the \"" << argumentName << "\" parameter ";
...
void showDeviceList(GenApi::CNodeMapRef &interfaceNodeMap)
{
  ...
  for (int64_t i = 0; i <= deviceSelector->GetMax(); ++i)
  {
    ...
    std::cout << i << ". DeviceModelName: "
      << deviceModelName->GetValue() << std::endl;
    std::cout << "   DeviceUserID: "
      << deviceUserId->GetValue() << std::endl;
    std::cout << "   DeviceSerialNumber: "
      << deviceSerialNumber->GetValue() << std::endl;
    std::cout << "   DeviceIPAddress: "
      << stringFromIp4(currentIp) << std::endl;
    std::cout << "   DeviceMACAddress: "
      << stringFromMac(currentMac) << std::endl;
    std::cout << "   DeviceID: "
              << deviceId->GetValue() << std::endl;
    std::cout << std::endl;
  }
}

GenTL::DEV_HANDLE openDeviceInteractively(
  ...
  GenTL::IF_HANDLE& interfaceHandle)
{
  ...
  Sample::InterfaceId interfaceId = consumer.findInterfaceByIndex(interfaces,
                                                                  input);
